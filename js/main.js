...
UI.getUserDOM = function(isDizhu,name,userIndex,sex,num){
	...
	$node.html('<div class="clock"><p>30</p></div><p class="name">'+name+'</p><div class="img"><div class="'+sex+'"></div></div><div class="pokerNum">'+numStr+'</div>');
	return $node;
}
/**
 * 获取数字字符串
 ...
 */
UI.win = function(){
	UI.showTips("游戏结束， 地主胜利");
	var $t = $('.wrap').addClass('win');
 ...
 */
UI.lose = function(){
	UI.showTips("游戏结束， 地主失败");
	var $t = $('.wrap').addClass('lost');
}
/**
 * 添加底牌
 */
...
UI.do出牌 = function(cards,id){
	...
	if(Tool.isArray(cards) && (len = cards.length) ){
		...
		window.curPoker = cards;
		window.lastChu = curUser.index;
...
UI.机器出牌 = function(){
	...
	if (window.lastChu == curUser.index){ //无人大过当前牌，又回到当前用户
		window.curPoker = []; //清空
		console.log("已过1轮");
	}


	var cards = AI.auto(curUser.pokers,curPoker);
	if (cards) {
...
UI.start = function(window){
	...
	curUser = users[0];
	users[0].isDizhu = !0;
	users[0].pokers = users[0].pokers.concat(dipai);
	users[0].pokerNum = 20;
